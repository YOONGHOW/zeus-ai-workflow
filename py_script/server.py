...
async def cron_scheduler_loop():
    while True:
        ...
        try:
            ...
            for t in tasks:
                print(f"Executing scheduled workflow {t.workflow_id}")
                # For this MVP, we only log since streaming execution requires an SSE client.
        except Exception as e:
...
async def startup_event():
    try:
        from dbconfig.db import engine
        from sqlalchemy import text
        with engine.begin() as conn:
